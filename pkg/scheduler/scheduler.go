...
func New(client clientset.Interface,
	...
	// 将 opts 中的配置参数，都设置到 options， 会覆盖 defaultSchedulerOptions 的默认值
	...
	if options.applyDefaultProfile {
		...
		// scheme 导入时，初始化函数以及执行，并设置了一些默认值，其中就包括了 设置默认的调度器名称，default-scheduler
		// 在执行 default是就将 默认值赋值到  versionedCfg
	...
	// 注册插件， 如果直接实现的插件，只需要在NewInTreeRegistry 下进行注册即可被调用
	...
	// 为每一个配置的调度器 new一个 framework，在framework会更具 opts 就行配置参数，opts 就是 frameworkruntime.*
	// 会配置 client，informer，lister kubeconfig，等
	...
	// 优先级队列， 这里队列内部包含了 activeQ 和backoffQ
...
func (sched *Scheduler) Run(ctx context.Context) {
	...
	// 我们需要在一个专用的goroutine中启动scheduleOne循环，因为scheduleOne函数将挂起从SchedulingQueue获取下一个项目。如果没有要调度的新pods，
	// 它将挂在那里，如果在这个goroutine中完成，它将阻止关闭SchedulingQueue，实际上会导致关闭时死锁。
