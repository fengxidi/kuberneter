...
// UpdateSnapshot获取缓存NodeInfo映射的快照。这在每个调度周期开始时调用。快照仅包括在调用此函数时未删除的节点。
// 保证快照中节点不为零。此函数跟踪NodeInfo的生成次数，并仅更新在创建快照后已更改的现有快照的条目。
// 【保证快照信息是最新的，将一切的更新都进行同步到快照】
func (cache *cacheImpl) UpdateSnapshot(nodeSnapshot *Snapshot) error {
	cache.mu.Lock()
	defer cache.mu.Unlock()

	// Get the last generation of the snapshot.
	// 获取最后一个版本的快照生成，如何保证就是最后一个版本：  nodeSnapshot.generation 在后续会进行重新赋值
	snapshotGeneration := nodeSnapshot.generation

	// NodeInfoList and HavePodsWithAffinityNodeInfoList must be re-created if a node was added
	// or removed from the cache.
	// 如果从缓存中添加或删除了节点，则必须重新创建NodeInfoList和HavePodsWithAffinityNodeInfoList。
	updateAllLists := false
	// HavePodsWithAffinityNodeInfoList must be re-created if a node changed its
	// status from having pods with affinity to NOT having pods with affinity or the other
	// way around.
	// 如果节点将其状态从具有关联的pod更改为不具有关联的pods，则必须重新创建HavePodsWithAffinityNodeInfoList。
	...
	// 如果节点将其状态从具有反亲和性的pod更改为不具有反亲和性或反之，则必须重新创建HavePodsWithRequiredAntiAffinityNodeInfoList。
	updateNodesHavePodsWithRequiredAntiAffinity := false
	// usedPVCSet must be re-created whenever the head node generation is greater than
	// last snapshot generation.
	// 每当头节点生成大于上次快照生成时，必须重新创建usedPVCSet。
	updateUsedPVCSet := false

	// Start from the head of the NodeInfo doubly linked list and update snapshot
	// of NodeInfos updated after the last snapshot.
	// 从NodeInfo双链表的开头开始，更新上次快照后更新的NodeInfos快照。
	for node := cache.headNode; node != nil; node = node.next {
		...
		if np := node.info.Node(); np != nil {
			...
			// 我们需要保留NodeInfo结构的原始指针，因为它在NodeInfoList中使用，我们可能不会更新。
			*existing = *clone
		}
	}
