...
func (p *PriorityQueue) flushBackoffQCompleted() {
	...
	for {
		...
		// pod在backoff中的时间是否在当前时间后，如果还没到时间，则直接退出，堆头没有到时间，说明后续的pod也都没有到时间
...
// 将超过 最大为调度时间[podMaxInUnschedulablePodsDuration] 的 pod 移动到 backoffQ 或者 activeQ
...
func (npm *nominator) delete(p *v1.Pod) {
	...
	// 通过nnn取值，就可用获取到 该节点上的 所有pods
	for i, np := range npm.nominatedPods[nnn] {
		// 如果节点上的pod与 假定pod是同一个
		if np.Pod.UID == p.UID {
			// 将该pod从节点确定的pod中删除
			npm.nominatedPods[nnn] = append(npm.nominatedPods[nnn][:i], npm.nominatedPods[nnn][i+1:]...)
			// 如果整个节点都没有值了，则删除该节点的key， （节点故障的情况下会出现这种情况）
