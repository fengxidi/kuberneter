...
func (f *frameworkImpl) RunFilterPluginsWithNominatedPods(ctx context.Context, state *framework.CycleState, pod *v1.Pod, info *framework.NodeInfo) *framework.Status {
	...
	// //在某些情况下，我们运行两次过滤器。如果节点的优先级大于或等于
	// 指定的pod，当这些pod被添加到PreFilter状态和nodeInfo时，我们运行它们。如果所有筛选器都在这一过程中成功，
	// 那么当未添加这些指定的pod时，我们再次运行它们。第二次通过是必要的，因为没有指定的pod，某些过滤器（如pod间亲和性）可能无法通过。
	// 如果没有为节点指定的pod，或者如果第一次运行过滤器失败，我们不会运行第二次。
	// 我们只考虑第一次运行中优先级相等或更高的pod，因为这些是当前的“pod”，必须向它们让步，
	// 而不占用运行它们的空间。如果当前的“pod”为较低优先级的pod释放资源，这是可以的。要求新的pod在两种情况下都是可调度的，
	// 这确保了我们做出了一个保守的决定：当指定的pod被视为正在运行时，像资源和跨pod反亲和性这样的过滤器更可能失败，
	// 而当指定的pods被视为未运行时，类似pod亲和性的过滤器更容易失败。我们不能仅仅假设指定的pod正在运行，
	// 因为它们现在没有运行，事实上，它们可能最终被调度到不同的节点。
	for i := 0; i < 2; i++ {
		stateToUse := state
		nodeInfoToUse := info
		if i == 0 {
...
// //addNominatedPods添加具有相同或更高优先级的pods
// //以在节点上运行。它返回1）是否添加了任何pod，
// //3）增强节点信息。
...
func (f *frameworkImpl) RunScorePlugins(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodes []*v1.Node) (ps framework.PluginToNodeScores, status *framework.Status) {
	...
	f.Parallelizer().Until(ctx, len(f.scorePlugins), func(index int) {
		...
		// 这里会运行 NormalizeScore
		// 用于在调度器计算 Node 排名之前修改分数
